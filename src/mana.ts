...
export function accessSecondFloor() {
  executeTask(async () => {
    try {
      ...
      console.log(res)
      payMana = true
      movePlayerTo({ newRelativePosition: Vector3.create(12.43, 10.92, 24.14) })
    } catch (error: any) {
...
export function accessThirdFloor() {
  executeTask(async () => {
    try {
      const provider = await createEthereumProvider()
      const requestManager = new EthConnect.RequestManager(provider)
      ...
      console.log(res)
      payMana2 = true
      movePlayerTo({ newRelativePosition: Vector3.create(4.01, 18.08, 6.51) })
    } catch (error: any) {
      console.log(error.toString())
    }
  })
